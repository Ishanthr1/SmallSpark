...
const CatBar = ({ th, selectedCat, selectedSub, onSelectCat, onSelectSub }) => {
    ...
    const [dropPos, setDropPos] = useState({ left: 0 });
    const timerRef = useRef(null);
    const outerRef = useRef(null);

    const handleMouseEnter = (cat, buttonEl) => {
        clearTimeout(timerRef.current);
        if (cat.subs.length > 0) {
            const rect = buttonEl.getBoundingClientRect();
            const outerRect = outerRef.current.getBoundingClientRect();
            setDropPos({ left: rect.left - outerRect.left });
        }
        setHovered(cat.id);
    };
    ...
    return (
        ...
        >
                ...
                {CATEGORIES.map(cat => {
                    ...
                    return (
                        <div key={cat.id} style={{ position: 'relative', flexShrink: 0 }}
                            onMouseEnter={e => handleMouseEnter(cat, e.currentTarget)}
                        >
            ...
            {/* Dropdown rendered OUTSIDE the scrollable div */}
            {hovered && hoveredCat?.subs.length > 0 && (
                <div style={{
                    position: 'absolute', top: '100%', left: dropPos.left,
                    ...
                    {hoveredCat.subs.map(sub => {
                        ...
                        return (
                            <button key={sub.name}
                                onClick={() => { onSelectCat(hoveredCat.id); onSelectSub(subActive ? null : sub.name); setHovered(null); }}
                                style={{
    ...
    return (
        ...
        >
            ...
            <div style={{flex: 1, minWidth: 0, display: 'flex', flexDirection: 'column'}}>
                ...
                {/* Bottom row */}
                <div
                    style={{display: 'flex', alignItems: 'center', gap: '0.5rem', marginTop: 'auto', flexWrap: 'wrap'}}>
                    <span style={{
        ...
        <div style={{
            ...
            <div style={{
                ...
                <div style={{position: 'relative'}}>
                    ...
                    <ChevronDown size={12} color={th.textMuted} style={{
                        position: 'absolute',
                        right: '0.5rem',
                        top: '50%',
            ...
            <div style={{flex: 1, overflowY: 'auto'}}>
                ...
                {!hasSearched && !loading && (
                    <div style={{
                        ...
                        <h2 style={{
                            ...
                            color: th.text,
                            marginBottom: '0.4rem'
                        }}>Discover Digital Businesses</h2>
                        <p style={{
                            fontSize: '0.84rem',
                            color: th.textMuted,
                            lineHeight: '1.5',
                ...
                {loading && (
                    <div style={{
                        ...
                        <p style={{fontSize: '0.85rem', color: th.textMuted}}>Searching...</p>
                    </div>
                )}
                    ...
                    <>
                        {businesses.length === 0 ? (
                            <div style={{
                                ...
                                <p style={{fontSize: '0.82rem', color: th.textMuted, marginBottom: '1rem'}}>Try a
                                    different search or clear your filters</p>
                                <button onClick={clearFilters} style={{
                            ...
                            <>
                                <div style={{
                                    ...
                                    {subcategory && <> in <span
                                        style={{fontWeight: '600', color: th.text}}>{subcategory}</span></>}
                                    {!subcategory && category !== 'all' && <> in <span
                                        style={{fontWeight: '600', color: th.text}}>{catLabel}</span></>}
